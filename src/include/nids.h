...
struct tcp_stream
{
  ...
  void *user;
  int client_to_server;                  //数据包方向
  void *match_parser;                     //匹配状态，回调函数中需要
};
...
//GTP_Header
typedef struct GTP_MANDATORY_HEADER
{
	unsigned version:3;					//version
	unsigned protocol_type:1;			//protocol_type
	unsigned reserved:1;				//reserved,always 0
	unsigned ext_header_flag:1;			//extension header flag
	unsigned seq_num_flag:1;			//seqence number flag
	unsigned npdu_num_flag:1;			//N_PDU number flag
	unsigned char message_type;			//message type
	unsigned short total_len;			//total len of the payload,including the extension headers
	unsigned int TEID;					//tunnel endpoint identifier (TEID)
}GTP_Mandatory_Header;

typedef struct GTP_OPTIONAL_HEADER
{
	unsigned short seq_num;				//seqence number
	unsigned char N_PDU_num;			//N_PDU number
	unsigned char next_ext_header_type;	//next extension header type
}GTP_Optional_Header;
